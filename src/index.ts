...
function from565(RGB565: number): RGBA {
...
function lerpColor(color1: RGBA, color2: RGBA, t: number) {
...
function interpolate565(color0: number, color1: number) {
...
function setPixel(x: number, y: number, colour: RGBA, imageData: Canvas.ImageData) {
...
function saveTexture(chunk: Texture) {
	const textureData = chunk.chunks[0] as TextureData;
	...
	if (texFormat === 21 || texFormat === 22) {
		...
		fs.writeFileSync(`out/${textureName}.png`, rawImage);
		...
		for (let blockIndex=0; blockIndex<blocks.length; blockIndex++) {
			...
			setPixel(blockX + 3 , blockY + 0, { ...dxtPalette[index0], A: (transparencyGrid[0][2] * 17) }, canvasData);
			setPixel(blockX + 2 , blockY + 0, { ...dxtPalette[index1], A: (transparencyGrid[0][3] * 17) }, canvasData);
			setPixel(blockX + 1 , blockY + 0, { ...dxtPalette[index2], A: (transparencyGrid[0][0] * 17) }, canvasData);
			setPixel(blockX + 0 , blockY + 0, { ...dxtPalette[index3], A: (transparencyGrid[0][1] * 17) }, canvasData);

			setPixel(blockX + 3 , blockY + 1, { ...dxtPalette[index4], A: (transparencyGrid[1][2] * 17) }, canvasData);
			setPixel(blockX + 2 , blockY + 1, { ...dxtPalette[index5], A: (transparencyGrid[1][3] * 17) }, canvasData);
			setPixel(blockX + 1 , blockY + 1, { ...dxtPalette[index6], A: (transparencyGrid[1][0] * 17) }, canvasData);
			setPixel(blockX + 0 , blockY + 1, { ...dxtPalette[index7], A: (transparencyGrid[1][1] * 17) }, canvasData);

			setPixel(blockX + 3 , blockY + 2, { ...dxtPalette[index8], A: (transparencyGrid[2][2] * 17) }, canvasData);
			setPixel(blockX + 2 , blockY + 2, { ...dxtPalette[index9], A: (transparencyGrid[2][3] * 17) }, canvasData);
			setPixel(blockX + 1 , blockY + 2, { ...dxtPalette[index10], A: (transparencyGrid[2][0] * 17) }, canvasData);
			setPixel(blockX + 0 , blockY + 2, { ...dxtPalette[index11], A: (transparencyGrid[2][1] * 17) }, canvasData);

			setPixel(blockX + 3 , blockY + 3, { ...dxtPalette[index12], A: (transparencyGrid[3][2] * 17) }, canvasData);
			setPixel(blockX + 2 , blockY + 3, { ...dxtPalette[index13], A: (transparencyGrid[3][3] * 17) }, canvasData);
			setPixel(blockX + 1 , blockY + 3, { ...dxtPalette[index14], A: (transparencyGrid[3][0] * 17) }, canvasData);
			setPixel(blockX + 0 , blockY + 3, { ...dxtPalette[index15], A: (transparencyGrid[3][1] * 17) }, canvasData);
		...
		fs.writeFileSync(`out/${textureName}.raw`, textureData.data);
	}
}

function parseFile(data: Buffer): TXDFile {
	const txdData = new PointerBuffer(data);
	...
	const infoChunk = parseChunk(txdData, false) as TXDInfo;
...
function parseChunk(data: PointerBuffer, hadInfo: boolean): TXDChunk | null {
	let chunk : TXDChunk = {
		id: -1,
		chunk_size: 0,
		rw_version: 0,
...
fs.writeFileSync("out.json", JSON.stringify(parsed, null, '\t'));
...
for (let chunk of parsed.chunks) {
	if (chunk.id == 21) {
		const texture = chunk as Texture;
		saveTexture(texture);
		for (let ct of chunk.chunks) {
			if (ct.id == 0x01) {
				const tData = ct as TextureData;
